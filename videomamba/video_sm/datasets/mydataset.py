...
class VideoClsDataset_sparse(Dataset):
    ...
    def __init__(self, anno_path, prefix='', split=' ', mode='train', clip_len=8,
        ...
        self.prefix = prefix
        self.face_analyzer = FaceAnalysis() # 假设这是加载RetinaFace模型的方式
        self.face_analyzer.prepare(ctx_id=0, det_size=(320, 320)) # 根据你的设置调整
          # 假定的人脸帧尺寸，根据实际情况调整
        self.default_face_height = 224
        self.default_face_width = 224
    ...
    def detect_faces(self, frame):
        ...
        if len(faces) > 0:
            ...
            self.last_face_frame = face_frame  # 更新最后一次检测到的人脸帧
            return face_frame
        else:
            if self.last_face_frame is not None:
                # 如果之前检测到过人脸，但这次没检测到，则使用上一帧的人脸并添加噪声
                noisy_face_frame = self.last_face_frame.copy()
                noise = np.random.randint(0, 256, self.last_face_frame.shape, dtype=np.uint8)
                noisy_face_frame = cv2.addWeighted(noisy_face_frame, 0.5, noise, 0.5, 0)
                return noisy_face_frame
            else:
                # 如果从未检测到人脸，则返回一个随机噪声图像
                noise_image = np.random.randint(0, 256, (self.default_face_height, self.default_face_width, 3), dtype=np.uint8)
                return noise_image
